...
void wallFollow(){
  ...
  if(counter==0){
    x=dof.calcGyro(dof.gx);
    y=dof.calcGyro(dof.gy);
  }
...
//??
  ...
  detectFire();
   ...
   dof.readGyro();
   if( y > dof.calcGyro(dof.gy) ) //APOSTASH APO TON MPROSTA DEKSIA LIGO ARISTERA
     {
      goLeft();
     }//else if
     
   dof.readGyro();
   if (y < dof.calcGyro(dof.gy) ) //APOSTASH APO TON PISW DEKSIA  LIGO ARISTERA  
     {
     goRight();
  ...
  if(readRightUS() > 35 && readRight2US() > 35 ) //KAI TA DUO DEIXNOUN KENO strofi
   ...
   if (readFrontUS() > 3000  || readRightUS()>3000 || readRight2US()>3000) //|| (readFrontUS() > 60 && readRight2US() < readRightUS()))  //LOGIKO LATHOS panw sto toixo h skedasi KANEI LIGO PISW 
   {
     moveBackward();
     delay(300);
     detectFire();
