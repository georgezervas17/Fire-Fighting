...
//??
 ...
 // detectFire();
   ...
   if( readRightUS() < 0 ) //APOSTASH APO TON MPROSTA DEKSIA LIGO ARISTERA
     {
      goLeft();
     }//else if
   
   if (readRightUS() > 0 ) //APOSTASH APO TON PISW DEKSIA  LIGO ARISTERA  
     {
     goRight();
  ...
  if(readRightUS() > 35 && readRight2US() > 35 ) //KAI TA DUO DEIXNOUN KENO strofi
   ...
   if (readFrontUS() > 300  || readRightUS()>300 || readRight2US()>300) //|| (readFrontUS() > 60 && readRight2US() < readRightUS()))  //LOGIKO LATHOS panw sto toixo h skedasi KANEI LIGO PISW 
   {
     moveBackward();
     delay(300);
     //detectFire();
